...
export const Popover: React.FC<PopoverProps> = ({
  ...
  const { products: choisenProducts } = useShoppingCart();
  const choisenProductsMap = new Map(
    choisenProducts.map(product => [product.uuid, product])
  );
  const products = useGetProductsQuery(undefined, {
    selectFromResult: ({ data: products = [] }) =>
      products.filter(product => choisenProductsMap.has(product.uuid)),
  });
